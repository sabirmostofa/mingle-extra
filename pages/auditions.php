...
$aud_id = $_POST['aud_id'];
echo $aud_author=$wpdb->get_var($wpdb->prepare("SELECT post_author FROM wp_posts where ID='$aud_id';"));
...
if(!preg_match('/\S/',$_POST['aud_video'])){
	...
	if(!$mngl_friend->is_friend($aud_author,$user_id))
	     $mngl_friend->create_both($aud_author,$user_id,'verified');
...
echo  '<div id="aud_des"></div>';
echo '<table class="naked">';
	
foreach($new_query->posts as $single)
echo "<tr><td>{$single->post_title}</td><td><a class='view_des' href='{$this_page_link}/?aud_id={$single->ID}'>View Description</a></td><td><a href='{$this_page_link}/?action=submit&id={$single->ID}'>Submit a video</a></td></tr>";
